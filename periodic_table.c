...
Periodic_Table* read_in_periodic_table(char *filename) {
	...
	if (fp == NULL) {
		printf(
				"Failed to open file; please make sure file exists, then try again.\n");
		free(pt);
		exit(1);
	}
...
Element* create_new_element(int atomic_number, char *name, char *abbrev,
		char *classification, char *properties) {
...
void add_new_element_to_periodic_table(Periodic_Table *pt) {
	...
	if (atomic_number < 1 || atomic_number > 188) {
		printf(
				"Sorry, your selection of atomic number is outside the valid range: 0 - 188\n");
		return;
	...
	Element *e = create_new_element(atomic_number, name, abbrev, classification,
			properties);
